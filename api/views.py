...
#@login_required(login_url='/api/admin_page/')
...
from api.serializers import CreditApplicationSerializer
...
#         serializer = CreditApplicationSerializer(data=request.data)
#         if serializer.is_valid():
#             credit_application = serializer.save(user=request.user, call_made=False)
#             return Response({'id': credit_application.id}, status=status.HTTP_201_CREATED)
#         return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
#     active_credits = Credit.objects.filter(payment_dates__gte=today).order_by('payment_dates')
...
from .forms import CreditConfirmationForm
from .models import CreditApplication, Credit
